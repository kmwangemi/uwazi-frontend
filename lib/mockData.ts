import type { Tender, Supplier, ProcuringEntity, Investigation, DashboardStats } from '@/types'
...
export const mockTenders: Tender[] = Array.from({ length: 50 }, (_, i) => ({
  ...
  procuring_entity: ['Nairobi County', 'Ministry of Health', 'Ministry of Roads', 'Ministry of ICT'][i % 4],
  county: KENYA_COUNTIES[i % KENYA_COUNTIES.length],
  category: ['Construction & Infrastructure', 'Medical Equipment & Supplies', 'Office Equipment & Furniture', 'Consultancy Services'][i % 4],
  submission_deadline: new Date(Date.now() + Math.random() * 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
  award_date: i % 2 === 0 ? new Date(Date.now() - Math.random() * 90 * 24 * 60 * 60 * 1000).toISOString().split('T')[0] : null,
  awarded_supplier_id: i % 2 === 0 ? Math.floor(Math.random() * 20) + 1 : null,
  awarded_supplier_name: i % 2 === 0 ? `Supplier ${Math.floor(Math.random() * 20) + 1}` : undefined,
  status: ['PUBLISHED', 'AWARDED', 'FLAGGED', 'UNDER_INVESTIGATION', 'COMPLETED'][Math.floor(Math.random() * 5)] as any,
  risk_score: Math.floor(Math.random() * 100),
  is_flagged: Math.random() > 0.7,
  corruption_flags: Math.random() > 0.6 ? [
    {
...
export const mockEntities: ProcuringEntity[] = Array.from({ length: 20 }, (_, i) => ({
  id: i + 1,
  entity_code: `ENT${String(i + 1).padStart(3, '0')}`,
  name: ['Ministry of Roads', 'Ministry of Health', 'Ministry of Education', 'Nairobi County', 'Kiambu County'][i % 5],
...
export const mockInvestigations: Investigation[] = Array.from({ length: 15 }, (_, i) => ({
  id: i + 1,
  ...
  opened_date: new Date(Date.now() - Math.random() * 180 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
  target_date: new Date(Date.now() + Math.random() * 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0],
  closed_date: i % 4 === 3 ? new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0] : null,
  ...
  outcome: i % 4 === 3 ? ['CONFIRMED_FRAUD', 'NO_FRAUD', 'INCONCLUSIVE'][Math.floor(Math.random() * 3)] as any : null
}))
...
export const mockPublicReports: PublicReport[] = [
  {
    ...
    description: 'Observed significant price inflation in tender TND/2024/001 for road construction. The quoted amount appears 300% above market rates.',
    county: 'Nairobi',
  ...
  },
  {
    ...
    description: 'The same supplier has been awarded 15 consecutive tenders in the last 6 months despite having higher bids than competitors.',
    county: 'Kiambu',
  ...
  },
  {
    ...
    description: 'Medical equipment valued at KES 5M was recorded as delivered but never found in the hospital inventory.',
    county: 'Mombasa',
  ...
  },
  {
    ...
    description: 'The procurement officer managing tender evaluation is related to the director of the winning supplier company.',
    county: 'Kisumu',
  ...
  },
  {
    ...
    description: 'The tender file for consultancy services lacks supporting documentation for the evaluation criteria.',
    county: 'Nakuru',
  ...
  },
  {
    ...
    description: 'Supplier submitted false ISO certifications and technical qualifications to win the tender.',
    county: 'Nairobi',
    ...
    severity: 'Medium',
    anonymous: true,
    rejectionReason: 'Insufficient evidence provided. Submitter requested to provide supporting documents.',
  },
]
