...
export default function EntitiesPage() {
  ...
  const entities = useMemo(() => {
    ...
    mockTenders.forEach(tender => {
      if (!entityMap.has(tender.procuring_entity)) {
        const entity = mockTenders.filter(
          t => t.procuring_entity === tender.procuring_entity,
        );
        entityMap.set(tender.procuring_entity, {
          id: tender.procuring_entity,
          name: tender.procuring_entity,
          county: tender.county,
          tenderCount: entity.length,
          totalSpent: entity.reduce((sum, t) => sum + t.amount, 0),
          averageRisk:
            entity.reduce((sum, t) => sum + t.risk_score, 0) / entity.length,
        });
      }
    });
