...
import { Plus, Eye, Flag, Clock, CheckCircle, AlertCircle, Search, AlertTriangle } from 'lucide-react'
...
export default function InvestigationsPage() {
  ...
  const [newInvestigationDialogOpen, setNewInvestigationDialogOpen] = useState(false)
  ...
  const getStatusIcon = (status: string) => {
    if (status === 'Open') return <AlertCircle className="h-4 w-4 text-blue-600" />
  ...
  const pendingPublicReports = mockPublicReports.filter((r) => r.status === 'Pending')
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <div className="flex gap-3">
          <Button variant="outline" size="sm" onClick={() => router.push('/investigations/pending-reports')}>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-yellow-900">
                  {pendingPublicReports.length} pending public report{pendingPublicReports.length !== 1 ? 's' : ''}
                </p>
                <p className="text-sm text-yellow-800">Awaiting review and conversion to investigations</p>
              </div>
            </div>
            <Button
              variant="default"
                ...
                <div className="flex items-start justify-between gap-2">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <h3 className="font-semibold text-gray-900 line-clamp-2">{investigation.title}</h3>
                    </div>
                    <p className="text-sm text-gray-600 mt-1">{investigation.caseNumber}</p>
                  </div>
