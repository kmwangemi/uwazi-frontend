          ...
          <div className='grid grid-cols-1 md:grid-cols-4 gap-4'>
            ...
            <Card className='p-4'>
              ...
              <p className='text-2xl font-bold'>
                {(
                  mockBids.reduce((sum, b) => sum + b.totalScore, 0) /
                  mockBids.length
                ).toFixed(1)}
              </p>
            </Card>
            <Card className='p-4'>
              ...
              <p className='text-2xl font-bold'>
                {formatCurrency(Math.min(...mockBids.map(b => b.biddingPrice)))}
              </p>
            </Card>
            <Card className='p-4'>
              <p className='text-sm text-gray-600 mb-1'>Highest Bid</p>
              <p className='text-2xl font-bold'>
                {formatCurrency(Math.max(...mockBids.map(b => b.biddingPrice)))}
              </p>
            </Card>
          </div>
          {/* Scoring Chart */}
          ...
          <Card className='p-6'>
            ...
            <ResponsiveContainer width='100%' height={250}>
              ...
              >
                ...
                <YAxis
                  label={{
                    value: 'Amount (Millions KSh)',
                    angle: -90,
                    position: 'insideLeft',
                  }}
                />
                <Tooltip
                  formatter={value => `KSh ${Number(value).toFixed(1)}M`}
                />
        ...
        <TabsContent value='details' className='space-y-4'>
          ...
          <div className='space-y-3'>
            {filteredBids.map((bid, index) => (
              <Card
                key={bid.id}
                className={`p-5 border-2 transition-all ${getRankColor(index)}`}
              >
                <div className='flex items-start justify-between mb-4'>
                  ...
                  <div className='flex gap-2'>
                    <Badge
                      className={getRecommendationColor(bid.recommendation)}
                    >
                      {bid.recommendation}
                    </Badge>
                ...
                <div className='grid grid-cols-2 md:grid-cols-5 gap-4 mb-4 pb-4 border-b border-gray-200'>
                  <div>
                    ...
                    <p className='font-semibold'>
                      {formatCurrency(bid.biddingPrice)}
                    </p>
                  </div>
                  <div>
                ...
                <div className='flex gap-2'>
                  <Button
                    ...
                    onClick={() =>
                      router.push(
                        `/dashboard/tenders/${params.id}/bids/${bid.id}`,
                      )
                    }
                  >
