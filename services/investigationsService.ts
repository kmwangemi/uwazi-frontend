...
import type { Investigation, ApiResponse, PaginationParams } from '@/types'

export const investigationsService = {
  getInvestigations: async (params: PaginationParams & Record<string, any>): Promise<ApiResponse<Investigation[]>> => {
    try {
      const response = await api.get<ApiResponse<Investigation[]>>('/api/investigations', { params })
  ...
  getInvestigationById: async (id: number): Promise<Investigation> => {
    try {
      const response = await api.get<Investigation>(`/api/investigations/${id}`)
      return response.data
    } catch (error) {
  ...
  closeInvestigation: async (id: number, outcome: string) => {
    try {
      const response = await api.post(`/api/investigations/${id}/close`, { outcome })
      return response.data
    } catch (error) {
  ...
  uploadEvidence: async (investigationId: number, file: File) => {
    try {
      ...
      const response = await api.post(`/api/investigations/${investigationId}/evidence`, formData, {
