...
import type { Tender, ApiResponse, PaginationParams, FilterParams } from '@/types'

export const tendersService = {
  getTenders: async (params: PaginationParams & FilterParams): Promise<ApiResponse<Tender[]>> => {
    try {
      const response = await api.get<ApiResponse<Tender[]>>('/api/tenders', { params })
      return response.data
    } catch (error) {
      // Return mock data for development
      const { page = 1, limit = 25, search, county, category, risk_level } = params
      ...
      if (risk_level) {
        ...
        filtered = filtered.filter(t => t.risk_score >= min && t.risk_score <= max)
      }
